...
The diagram on the following page shows the full lifecycle as a single map.
Every box carries the section number (\S) of the page that explains it in
detail. Box shapes and colours encode the kind of step:
...
% =====================================================================
%  FULL-PAGE FLOWCHART (landscape)
% =====================================================================
\begin{landscape}
\thispagestyle{fancy}
\begin{center}
{\large\bfseries The ParadePaard Employee Lifecycle \textemdash{} Full Flow}\\[2mm]
\end{center}
...
\node[terminal, above=14mm of apply] (adminstart) {Admin direct\\onboarding};

% deny branch (below accept)
\node[email,  below=12mm of accept] (denymail) {Denial email\\(\S\ref{sec:emails})};
\node[reject, below=10mm of denymail] (denied) {Application\\\code{DENIED}};
...
\node[reject, above=12mm of profok] (changes) {Changes requested\\\code{CHANGES_REQUESTED}};
\node[reject, below=12mm of profok] (onbreject) {Onboarding\\\code{REJECTED}};

% contract spine (second row, going right)
\node[process, below=24mm of onbreject, xshift=-30mm] (contract) {Contract draft\\(\S\ref{sec:contract})};
...
\node[reject, above=12mm of finalize] (creject) {Contract\\\code{REJECTED}};
\node[process, right=of finalize] (cview) {Employee contract\\view (\S\ref{sec:contractview})};

% work spine (third row)
\node[process, below=24mm of contract, xshift=0mm] (planning) {Shift planning\\(\S\ref{sec:planning})};
\node[decision, right=of planning] (shiftok) {Shift\\accepted?};
\node[reject, above=12mm of shiftok] (declined) {Shift\\\code{CANCELLED}};
...
% payroll spine (fourth row)
\node[process, below=24mm of planning] (paygen) {Payslip generation\\(\S\ref{sec:payslipgen})};
\node[decision, right=of paygen] (zero) {Zero-hour\\on-call?};
\node[reject, above=12mm of zero] (skip) {Period skipped\\(no payslip)};
\node[decision, right=of zero] (datok) {Data\\complete?};
\node[reject, below=12mm of datok] (attention) {\code{NEEDS_ATTENTION}\\auto-flagged};
...
\node[terminal, right=of release] (gotpay) {Payslip\\received};
...
\draw[flow] (cview.east) to[out=0,in=0] (planning.east);
