...
The user can refresh the contract list and open the related employee profile from any contract row. The employee profile contains the active contract card, PDF download, send, finalize, and reject actions when the user's permissions allow those actions. Contract PDF generation now has the correct service connection in Docker, so downloading a contract from the profile can fetch the employee profile data needed to build the PDF.
