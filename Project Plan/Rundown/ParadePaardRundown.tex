...
The employee can move forward only after completing the required fields for the current section. The Back button lets the employee return to earlier sections. When the employee finishes setup, the frontend submits the setup details and then uploads the ID document image. If the image upload fails, the employee stays on onboarding and sees the upload error so they can retry instead of being sent away as if setup succeeded.
