...
The employee onboarding page is the first self-service page for a newly invited employee. It is built as a two-step card called Finish Your Setup. Step 1 collects address information. The employee fills in street, house number, optional suffix, postal code, city, and country. The house number and suffix fields sit next to each other as separate input cells with clear spacing between them, so the user can tell where one value ends and the other begins. Step 2 collects the IBAN. The step indicator makes it clear that onboarding is short: Address first, IBAN second.
...
When the employee finishes setup, the account moves to \code{PENDING_PROFILE_REVIEW}. The employee has completed their own part, but the account is not treated as active payroll-ready work until management reviews the profile and the contract flow is completed. Existing project databases now accept this profile-review status, so finishing setup no longer fails with a generic bad-request message after the employee submits their address and IBAN.
...
The main management work pages now include an arrow-style Back to management control near the page title. This gives users a clear way to return from users, onboarding, onboarding review, contracts, planning, clients, travel claims, payslip review, all-payslips view, and management payslip details back to the Management Dashboard. Company settings also returns to Management when it is opened from the Management Dashboard.
...
The page is organized into Profile, Timesheets, and Planning tabs. The Profile tab shows personal information, work details, active contract details, employee tax profile, roles and access, address, and bank details. The active contract card now also shows contract status, contract payment frequency, signature timestamps, finalization timestamps, and review comments. If the viewer has the right contract permissions, they can send a draft contract to the employee, finalize an employee-signed contract, or reject it with a review comment. If the viewer also has role permissions, they can add or remove roles. If they have user-management permissions, they can save management fields that are editable for them.
...
The page shows contract rows with employee name, function, contract status, start date, end date, and an action. A status filter lets the user switch between all contracts, contracts needing review, draft or sent contracts, finalized contracts, and rejected or expired contracts. Contract statuses are displayed in readable labels instead of raw database values.

The user can refresh the contract list and open the related employee profile from any contract row. The employee profile contains the active contract card, PDF download, send, finalize, and reject actions when the user's permissions allow those actions.
...
The main actions on this page are Contract PDF download and contract signing. If a current contract exists, the user can download it from the Contract PDF row. If the contract is waiting for employee signature, the user also sees a Sign contract button. Below the current details, the page shows a compact contract history with date range, payment frequency, and status for each contract. If the contract cannot be loaded, signed, or downloaded, the page shows an error message.
...
The employee then logs in. If a password reset is required, that happens first. If the employee is still \code{PENDING_SETUP}, they are sent to the onboarding page. There they complete address and IBAN information. When setup is finished, the employee moves to profile review. Management can then send the draft contract to the employee. The employee can view and sign their own contract from Account Employment details. Management can finalize the signed contract or reject it with a comment.
...
Contract statuses drive contract-owned payroll. \code{DRAFT} is a contract being prepared. \code{SENT_TO_EMPLOYEE} means the employee can view and sign it. \code{EMPLOYEE_SIGNED} means it is waiting for management review. \code{FINALIZED} means it can be used for payroll. \code{REJECTED} means management sent it back with a comment. \code{SIGNED} remains visible for older contract rows.
