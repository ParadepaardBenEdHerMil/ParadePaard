...
The page shows grouped sections for personal details, work interest, and confirmation. Personal details include full first names, surname, email address, phone number, date of birth, gender, nationality, current city, and current country. Work interest includes role interest, contract preference, availability or start date, a short motivation, and an optional CV upload.

The user can fill in the required fields, choose a contract preference such as on-call, fixed hours, temporary event work, or no preference, attach a CV if they want, and submit the application. Before submitting, the user must confirm that ParadePaard may contact them about the application and that the information is accurate.

The page intentionally does not ask for sensitive onboarding information such as bank details, BSN or tax ID, identity document numbers, or ID images. After submission, the page shows an ``Application submitted'' confirmation and explains that sensitive onboarding information is only requested later if the application continues. If submission fails, the form shows an error message so the applicant can try again.
...
Once the user is signed in, access rules keep the flow clean. A pending setup user is redirected to onboarding. A user with \code{PENDING_PROFILE_REVIEW} is redirected to onboarding and sees the waiting state instead of the normal dashboard. A signed-out user is redirected to login. A user missing a required permission is sent back to dashboard. Management pages use permission guards such as \code{CAN_VIEW_USERS}, \code{CAN_MANAGE_PLANNING}, \code{CAN_MANAGE_TIMESHEETS}, \code{CAN_VIEW_ALL_PAYSLIPS}, and company or role permissions. This creates a guided path through the app instead of exposing pages the user cannot use.
