...
The user can refresh the contract list and open the related employee profile from any contract row. The employee profile contains the active contract card, PDF download, send or resend email, finalize, and reject actions when the user's permissions allow those actions. Contract PDF generation now has the correct service connection in Docker, so downloading a contract from the profile can fetch the employee profile data needed to build the PDF.
...
The main actions on this page are Contract PDF download and contract signing. If a current contract exists, the user can download it from the Contract PDF row. Contract PDF download uses the contract service together with employee profile data from the user service, so the downloaded document can include the employee name, address, contact details, and IBAN. If the contract is waiting for employee signature, the user also sees a Sign contract button. Below the current details, the page shows a compact contract history with date range, payment frequency, and status for each contract. If the contract cannot be loaded, signed, or downloaded, the page shows an error message.
